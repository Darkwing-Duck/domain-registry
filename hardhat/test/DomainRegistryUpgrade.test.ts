...
describe("DomainRegistry upgrade", function () {
  it("Upgrade", async function () {
    ...
    const DomainRegistryProtoV1 =
      await ethers.getContractFactory("DomainRegistry");
    let domainRegistry = await upgrades.deployProxy(DomainRegistryProtoV1, [
      owner.address,
      registrationPrice,
    ]);

    expect(await ethers.provider.getBalance(domainRegistry)).to.equal(
      targetRegistryBalance,
    );
    ...
    targetRegistryBalance +=
      BigInt(secondLevelSubDomains.length) * registrationPrice;
    expect(await ethers.provider.getBalance(domainRegistry)).to.equal(
      targetRegistryBalance,
    );
    ...
    targetRegistryBalance +=
      BigInt(thirdLevelSubDomains.length) * registrationPrice;
    expect(await ethers.provider.getBalance(domainRegistry)).to.equal(
      targetRegistryBalance,
    );
